...
function App() {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      // Create custom canvas for proper image combination
      ...
      if (hasBothImages) {
        ...
        // Set canvas size based on image dimensions and scale mode
        let canvasWidth, canvasHeight
        if (scaleMode === '1:1 Ratio') {
          canvasWidth = Math.max(beforeImg.naturalWidth, afterImg.naturalWidth)
          canvasHeight = Math.max(beforeImg.naturalHeight, afterImg.naturalHeight)
        } else {
          // Scale to fit - use container dimensions
        ...
        if (scaleMode === '1:1 Ratio') {
          imgWidth = beforeImg.naturalWidth
          imgHeight = beforeImg.naturalHeight
        ...
        const img = new Image()
        const imageData = beforeImage || afterImage
  ...
  }, [beforeImage, afterImage, hasBothImages, scaleMode, sliderPosition, showNotification])
