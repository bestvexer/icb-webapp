...
const AppLogo = () => {
  ...
  const [logoSrc, setLogoSrc] = useState('/icbench-logo.jpeg')
  
  if (logoError) {
    // Text-based fallback logo
    return (
      <div className="app-logo-fallback">
        <span className="logo-text">ICBENCH</span>
      </div>
    )
  }
  ...
  return (
    <img 
      ...
      onError={(e) => {
        ...
        if (logoSrc === '/icbench-logo.jpeg') {
          console.log('Trying old logo fallback...')
      ...
      onLoad={() => console.log(`ICBENCH logo loaded successfully from ${logoSrc}`)}
    />
  )
}
